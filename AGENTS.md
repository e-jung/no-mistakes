...
- **Swift provider (`coverage_swift.go`):** delegates to a Mac build executor over SSH because this VPS has no Swift toolchain. Active only when both a Swift manifest (`Package.swift` or `*.xcodeproj`/`*.xcworkspace`) is present AND `NM_SWIFT_SSH_HOST` names the Mac. Two build modes selected by `NM_SWIFT_BUILD_MODE` (default `swiftpm`): `swiftpm` runs `swift test --enable-code-coverage` then cats the JSON export whose path `swift test --show-code-coverage-path` names (on SwiftPM 5.8+ this is already the final `llvm.coverage.json.export`; parses `data[].files[].segments` → blocks spanning `[seg.line, nextSeg.line-1]`); `xcode` runs `xcodebuild test -enableCodeCoverage` + per-file `xcrun xccov view --file --json` (parses `coveredLines`/`uncoveredLines` → 1-line blocks), and **errors clearly when `xcodebuild -version` fails** (respects `xcode-select`, works for versioned installs like `/Applications/Xcode-26.5.0.app`; still gated). Remote flow syncs the head SHA via `git fetch origin && git checkout $HEAD_SHA` (refuses a dirty tree — never hard-resets) and uses `ssh host bash -l` over stdin so the Mac's Homebrew/CLT tools are on PATH. **Path bridge:** the Mac emits Mac-relative source paths in the JSON, but `ParseBlocks` receives the LOCAL workDir; `RunCoverage` bridges the two via `strings.ReplaceAll(raw, remotePath, sctx.WorkDir)` before returning, so `toRepoRelPOSIX` strips the prefix and block keys match `git diff --name-only` output (without this, keys are absolute Mac paths and the empty-blocks fallback fires a false positive on every changed file). Config knobs: `NM_SWIFT_SSH_HOST`, `NM_SWIFT_REMOTE_PATH`, `NM_SWIFT_BUILD_MODE`, `NM_SWIFT_SCHEME`, `NM_SWIFT_PROJECT`, `NM_SWIFT_SSH_OPTS`. Note: on a CLT-only Mac (no Xcode.app), `swift test` still fails because the XCTest/Swift Testing modules ship with Xcode.app — so both modes effectively require Xcode.app to be installed for end-to-end coverage collection, just via different code paths.
